  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await addDoc(collection(db, 'purchase_orders'), {
        ...
        date: new Date().toISOString().split('T')[0], status: 'draft', items, totalAmount: total,
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="relative w-full max-w-5xl bg-white rounded-[2.5rem] shadow-2xl flex flex-col max-h-[90vh] overflow-hidden border border-slate-200">
        <div className="p-10 border-b border-slate-100 flex justify-between items-center bg-[#1E3A5F] text-white">
          <div>
        ...
        <div className="flex-1 overflow-hidden flex flex-col lg:row">
          <div className="w-full lg:w-1/2 p-10 overflow-y-auto border-r border-slate-100 space-y-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  <div className="flex gap-2">
                    <input 
                      className={`flex-1 p-4 bg-slate-50 border ${poError ? 'border-red-400 ring-4 ring-red-50' : 'border-slate-200'} rounded-2xl font-bold font-mono text-[#1E3A5F] focus:ring-4 focus:ring-blue-50 outline-none transition-all`} 
                      value={poNumber} 
                    ...
                    <button 
                      ...
                      className="w-14 h-14 flex items-center justify-center bg-slate-100 text-slate-600 rounded-2xl hover:bg-white hover:text-blue-600 hover:shadow-lg border border-transparent hover:border-blue-100 transition-all active:scale-95 group"
                ...
                <div>
                  ...
                  <select 
                    className="w-full p-4 bg-slate-50 border border-slate-200 rounded-2xl font-bold text-[#1E3A5F] focus:ring-4 focus:ring-blue-50 outline-none transition-all appearance-none" 
              ...
              <div className="grid grid-cols-1 gap-3 max-h-96 overflow-y-auto pr-2 no-scrollbar">
                {catalog.filter(p => (p.name || '').toLowerCase().includes(searchTerm.toLowerCase()) || (p.sku || '').toLowerCase().includes(searchTerm.toLowerCase())).map(p => (
                  <div key={p.id} className="p-4 border border-slate-100 rounded-2xl flex justify-between items-center hover:bg-slate-50 hover:border-slate-200 cursor-pointer transition-all group" onClick={() => setItems([...items, {...p, quantity: 1, price: p.defaultPrice || 0}])}>
                    <div>
                      <p className="font-bold text-[#1E3A5F] text-sm group-hover:text-blue-600 transition-colors uppercase">{p.name}</p>
                      <p className="text-[10px] text-slate-400 font-mono tracking-tighter mt-0.5">{p.sku}</p>
                    </div>
                    <div className="text-right">
                      <p className="text-xs font-black text-[#1E3A5F]">{p.defaultPrice?.toLocaleString()} DZD</p>
          ...
          <div className="w-full lg:w-1/2 p-10 bg-slate-50/50 flex flex-col">
            <div className="flex items-center justify-between mb-6">
              <label className="text-[10px] font-black uppercase text-slate-400 tracking-widest">Articles de la commande</label>
              <span className="text-[10px] font-black bg-blue-100 text-blue-600 px-2 py-0.5 rounded uppercase">{items.length} Lignes</span>
            </div>
              ...
              {items.length === 0 ? (
                ...
                <div key={i} className="p-5 bg-white border border-slate-200 rounded-2xl shadow-sm flex items-center justify-between group">
                  <div className="flex-1 mr-4">
                    <p className="font-bold text-sm text-[#1E3A5F] truncate">{item.name}</p>
            ...
            <div className="pt-8 border-t border-slate-200">
              <div className="flex justify-between items-center mb-8">
                <div>
                  <span className="font-black uppercase text-[10px] text-slate-400 tracking-[0.2em] block mb-1">Montant Total Net</span>
                  <span className="text-3xl font-black text-[#1E3A5F] font-mono tracking-tighter">{total.toLocaleString()} <span className="text-sm font-sans text-slate-400">DZD</span></span>
                </div>
              </div>
              <button 
